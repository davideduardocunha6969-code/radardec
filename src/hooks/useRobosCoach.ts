...
export function useCreateRoboCoach() {
  ...
  return useMutation({
    mutationFn: async (data: { nome: string; descricao?: string; instrucoes: string; instrucoes_detector?: string; tipo?: string }) => {
...
export function useUpdateRoboCoach() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (data: { id: string; nome?: string; descricao?: string; instrucoes?: string; instrucoes_detector?: string; ativo?: boolean }) => {
      const { id, ...rest } = data;
