              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {colaboradoresAtraso.map((colaborador, index) => (
                    <div
                      ...
                      className="flex items-center gap-2 p-2 rounded-md cursor-pointer hover:bg-muted/50 transition-colors border border-transparent hover:border-border"
                      onClick={() => handleColaboradorClick(colaborador)}
                    >
                      {getMedalha(index) ? (
                        <span className="text-lg w-6 text-center">{getMedalha(index)}</span>
                      ) : (
                        <span className="text-xs text-muted-foreground w-6 text-center">{index + 1}º</span>
                      )}
                      <span className="flex-1 text-sm truncate" title={colaborador.nome}>
                        {colaborador.nome}
                      </span>
