...
serve(async (req) => {
  ...
  try {
    console.log('Fetching Trabalhista data (parallel)...');

    const [iniciaisData, atividadesData] = await Promise.all([
      fetchSheetData(GIDS.iniciais),
      fetchSheetData(GIDS.atividades),
    ]);
    ...
    const atividades = atividadesData.slice(1).map((row) => ({
      ...
      responsavel: row[10] || '',
      cliente: row[11] || '',
      numeroProcesso: row[12] || '',
    })).filter((a) => a.tipoTarefa);
    ...
    iniciais.forEach((i) => {
      if (i.responsavel) stats.iniciaisPorResponsavel[i.responsavel] = (stats.iniciaisPorResponsavel[i.responsavel] || 0) + 1;
      if (i.tipoInicial) stats.iniciaisPorTipo[i.tipoInicial] = (stats.iniciaisPorTipo[i.tipoInicial] || 0) + 1;
      if (i.semana) stats.iniciaisPorSemana[i.semana] = (stats.iniciaisPorSemana[i.semana] || 0) + 1;
      if (i.ano) stats.iniciaisPorAno[i.ano] = (stats.iniciaisPorAno[i.ano] || 0) + 1;
      if (i.profissao) stats.iniciaisPorProfissao[i.profissao] = (stats.iniciaisPorProfissao[i.profissao] || 0) + 1;
      if (i.situacao) stats.iniciaisPorSituacao[i.situacao] = (stats.iniciaisPorSituacao[i.situacao] || 0) + 1;
    ...
    const responseData = { iniciais, atividades, stats };
